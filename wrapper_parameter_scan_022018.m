...
for ii=1:n
    ...
    const3=0.6814; %constant coefficient for G3 %0.1-10 %0.6814
    %const4=10+rand*(1500-10); %constant coefficient for G4 %10-1500 %141.4998
    d_A=0.05+rand*(2-0.05); %0.3;%%G5 %0.05-2 %0.3
    d_L=0.0144;%0.01+rand*(0.05-0.01); %G6,G10,G11,G12 %0.005-0.02 %0.0144
    d_T=0.3;%0.1+rand*(1-0.1); %G8 %0.3
    KC=0.001+rand*(0.005-0.001); %0.0025; % 0.001-0.005 %cell diffusion coefficient %0.0025; kc can be change experimentally by changing the concentration of agar
    KT=50+rand*(5000-50); %0-5000 %G9,G10
    kD=0.037;%0.01+rand*(0.05-0.01); %G11 %0.037;
    KP=50+rand*(5000-50); %0-5000 %G12 nv 
...
%      fix=1/domainR^2;
    ...
    param.G2=const2;%.3;
    param.G3=const3*L_pam^3/alpha_c*fix;%const3*L_pam^3/alpha_c*fix;%0.0145/pi*fix;%0.0046;
    G4=rand*(10-1e-4)+1e-4; %0-10
    param.G4=G4/alpha_c*fix;   % 0-10 %const4*L_pam^3/alpha_c*fix;%3/pi*fix;
...
% 	[pks,locs,w2,p2]=findpeaks(totalmcherry); 
	if size(pks,1)>12 || neg_v>0  %%do not record the results if distribution has negative value or the peak values are too much (probably artifact)
		continue;
	end
    %end
    ...
    norm_idx_params=[10 5 2000 10 5 5 8000 9000 0.005 5000 5000 2.0 3.0]; %9 paameters
    m_parameters=[ G4 param.alpha param.beta param.Kphi param.exp_phi alpha_c alpha_T alpha_L KC KT KP d_A domainR]./norm_idx_params;
