...
if __name__ == "__main__":
    ...
    with open('data_031618_100000.csv') as csvfile1:
        mpg1 = list(csv.reader(csvfile1))
        results1=np.array(mpg1).astype("float")
        
    with open('data_031618_validation40000.csv') as csvfile2:
        mpg2 = list(csv.reader(csvfile2))
        results2=np.array(mpg2).astype("float")
    ...
    train_set = results1[:train_size,0:13], results1[:train_size,13:514], results1[:train_size,-2:-1] #parameters,distribution,peak value
    valid_set = results2[-valid_size:,0:13], results2[-valid_size:,13:514], results2[-valid_size:,-2:-1]


    train_mode = True
    mymodel = Model("save_j1", train_mode=train_mode, input_dim=13, T=501)
    if train_mode == True:
